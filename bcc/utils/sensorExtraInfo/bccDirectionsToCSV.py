...
url = "http://www.overpass-api.de/api/xapi?way[bbox=-2.1691,52.3088,-1.5930,52.6801][sensor_ref:lanes=*]"
r = requests.get(url)
...
url = "http://butc.opendata.onl/UTMC flow.xml"
r = requests.get(url)
print len(r.content)
#Download the node information from Overpass (we could probably reuse the way stuff from earlier) 
root = ET.fromstring(r.content)
urlosm = "http://www.overpass-api.de/api/xapi?node[bbox=-2.1691,52.3088,-1.5930,52.6801][monitoring=traffic]"
...
f = open(today()+"sensorDirections.csv", "wb")
lines = []
