...
Objectives: 
    - Push existing BCC Traffic records in 5 traffic-related collections to Local VM mongoDB 
    - Push ALL the existing records on UoW server mongoDB that were inserted before 30 minutes ago since the last insertion 
    time (based on "insertion_datetime" attribute)
    - Notes: 
        - This script acts a bulk write to insert all records and does not have a start timestamp threshold
        - This script should be run once only to migrate most of the records to Local VM
        - The script that should run as a cron job (e.g. every 30 minutes) will write new records in a time window 
          (e.g. 3 hours). For this, use pushBccRecordsTimeWindow.py
"""
...
def establish_connection_local_output(collection_name):
    try:
        # establish a connection to the collection
        host = 'localhost'
        port = 27017
...
def insert_new_records(input_collection, output_collection, collection_str):
    try:
        #get the last insertion time for the collection
        last_inserted_doc = input_collection.find({}).sort('insertion_datetime', pymongo.DESCENDING).limit(1)
        last_insertion_time = last_inserted_doc[0]['insertion_datetime']
        #log last insertion time        
        log_message = "Last insertion datetime in " + collection_str + ": " + str(last_insertion_time)
        logging.info(log_message)
        #print for testing
#        print dumps(last_inserted_doc[0], indent=2, default=json_util.default)        
        
        #get the datetime window
        end_date_time = last_insertion_time - datetime.timedelta(minutes = 30)
        start_date_time = end_date_time - datetime.timedelta(days = 365)
#        print for testing
#        print str(last_insertion_time)
    ...
    try:
        ...
        for collection in collections:
            if str(collection) not in bcc_collections:
                ...
                output_col = establish_connection_local_output(str("roadSensorValue"))
